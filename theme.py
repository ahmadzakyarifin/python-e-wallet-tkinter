...
def draw_icon(canvas, x, y, type, color):
    w = 3
    if type == "plus": 
        canvas.create_line(x-10, y, x+10, y, fill=color, width=w, capstyle="round")
        canvas.create_line(x, y-10, x, y+10, fill=color, width=w, capstyle="round")
    elif type == "arrow_r": 
        canvas.create_line(x-8, y, x+8, y, fill=color, width=w, capstyle="round")
        canvas.create_line(x+2, y-6, x+8, y, fill=color, width=w, capstyle="round")
        canvas.create_line(x+2, y+6, x+8, y, fill=color, width=w, capstyle="round")
    elif type == "arrow_d": 
        canvas.create_line(x, y-8, x, y+8, fill=color, width=w, capstyle="round")
        canvas.create_line(x-6, y+2, x, y+8, fill=color, width=w, capstyle="round")
        canvas.create_line(x+6, y+2, x, y+8, fill=color, width=w, capstyle="round")
    elif type == "dots": 
        for o in [-6, 0, 6]: canvas.create_oval(x+o-2, y-2, x+o+2, y+2, fill=color, outline="")
    elif type == "phone": 
        canvas.create_rectangle(x-7, y-11, x+7, y+11, outline=color, width=2)
        canvas.create_line(x-3, y+8, x+3, y+8, fill=color, width=2)
    elif type == "lightning": 
        canvas.create_line(x+3, y-10, x-5, y, fill=color, width=2)
        canvas.create_line(x-5, y, x+5, y, fill=color, width=2)
        canvas.create_line(x+5, y, x-3, y+10, fill=color, width=2)
